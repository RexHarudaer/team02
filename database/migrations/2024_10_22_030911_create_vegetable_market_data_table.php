...
class CreateVegetableMarketDataTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('vegetable_market_data', function (Blueprint $table) {
            ...
            $table->string('type', 50)->comment('類別'); // 類別
            $table->string('category', 50)->comment('植物名稱'); // 植物名稱
            $table->double('plant_picture', 5, 2)->comment('植物圖片價格'); // 植物圖片價格
            $table->double('total_average_price', 5, 2)->comment('總平均價格'); // 總平均價格
            $table->integer('total_yield')->comment('總產量'); // 總產量
            $table->string('market', 50)->comment('市場名稱'); // 市場名稱
            $table->double('average_price', 5, 2)->comment('市場的平均價格'); // 市場的平均價格
